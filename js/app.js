...
window.saveDontShowHands = function(checked) {
  if (checked) localStorage.setItem('digimaster_hands_seen', 'true');
  else localStorage.removeItem('digimaster_hands_seen');
};

// ---- Welcome Modal (unificado com lessons.js) ----
function initWelcome() {
  // lessons.js já define checkWelcome() com a chave 'digimaster_hide_welcome'
  // Esta função é chamada como fallback — deixa checkWelcome cuidar disso
}
window.closeWelcome = function() {
  const overlay = document.getElementById('welcomeOverlay');
  if (overlay) { overlay.classList.remove('open'); overlay.style.display = 'none'; }
};
window.saveDontShow = function(checked) {
  if (checked) localStorage.setItem('digimaster_hide_welcome', '1');
  else localStorage.removeItem('digimaster_hide_welcome');
};
window.showWelcome = function() {
  const overlay = document.getElementById('welcomeOverlay');
  if (overlay) { overlay.style.display = ''; overlay.classList.add('open'); }
};
...
function navigateTo(screenId) {
  ...
  if (screenId === 'ranking') loadRanking('wpm', document.querySelector('.rank-filter-btn'));
  if (screenId === 'admin') loadAdminData();
  if (screenId === 'practice') setTimeout(setupSession, 100);
}
...
function startTimers() {
  ...
  timerInterval = setInterval(() => {
    ...
    if (remaining === 10 && !lastAlertPlayed) {
...
function endSession() {
  ...
  if (typeof saveSessionCloud === 'function') {
    saveSessionCloud({ wpm, accuracy, errors: totalErrors, words: wordCount, level: currentLevel });
  }

  checkLessonCompletion(wpm, accuracy);
...
function handleKeydown(e) {
  if (document.getElementById('screen-practice')?.classList.contains('active')) {
    ...
    if (document.activeElement !== input && !inputLocked) {
      if (e.key.length === 1 && !e.ctrlKey && !e.altKey && !e.metaKey) focusInput();
    }
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const container = document.querySelector('.text-display-container');
  if (container) container.addEventListener('click', focusInput);

  document.addEventListener('keydown', handleKeydown);

  navigateTo('home');
  setTimeout(checkWelcome, 200);
  initWelcome();

  // Anima ring com melhor WPM histórico
  setTimeout(() => {
