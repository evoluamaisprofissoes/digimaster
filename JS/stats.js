...
function clearStats() {
  if (confirm('Tem certeza que deseja apagar o histórico local de sessões?')) {
...
function renderWpmChart(sessions) {
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    ctx.fillStyle = '#4a5a72'; ctx.font = '10px monospace'; ctx.textAlign = 'right'; ctx.fillText(val, pad.left - 6, y + 4);
  }
  const points = wpms.map((v, i) => ({ x: pad.left + (i / (wpms.length - 1)) * chartW, y: pad.top + chartH - ((v) / (maxV || 1)) * chartH }));
  const grad = ctx.createLinearGradient(0, pad.top, 0, pad.top + chartH);
  grad.addColorStop(0, 'rgba(0,212,255,0.3)'); grad.addColorStop(1, 'rgba(0,212,255,0)');
  ctx.beginPath(); points.forEach((p, i) => i ? ctx.lineTo(p.x, p.y) : ctx.moveTo(p.x, p.y)); ctx.lineTo(points.at(-1).x, pad.top + chartH); ctx.lineTo(points[0].x, pad.top + chartH); ctx.closePath(); ctx.fillStyle = grad; ctx.fill();
  ctx.beginPath(); ctx.strokeStyle = '#00d4ff'; ctx.lineWidth = 2.5; points.forEach((p, i) => i ? ctx.lineTo(p.x, p.y) : ctx.moveTo(p.x, p.y)); ctx.stroke();
  points.forEach((p, i) => { ctx.beginPath(); ctx.arc(p.x, p.y, 4, 0, Math.PI * 2); ctx.fillStyle = '#00d4ff'; ctx.fill(); ctx.strokeStyle = '#0a0e17'; ctx.lineWidth = 2; ctx.stroke(); ctx.fillStyle = '#7a8ba8'; ctx.font = '9px monospace'; ctx.textAlign = 'center'; ctx.fillText(i + 1, p.x, pad.top + chartH + 18); });
}
...
function renderAccBars(sessions) {
  ...
  buckets.forEach((b, i) => {
    ...
    group.innerHTML = `<div class="acc-bar" style="height:${Math.max(pct, 4)}px;background:${counts[i] > 0 ? 'var(--accent)' : 'rgba(0,212,255,0.15)'}"></div><div class="acc-bar-label">${b.label}</div>`;
...
function renderHistory(sessions) {
  ...
  tbody.innerHTML = reversed.slice(0, 50).map((s, i) => `
    <tr>
      ...
      <td>${new Date(s.date).toLocaleDateString('pt-BR', { day:'2-digit', month:'2-digit', year:'2-digit', hour:'2-digit', minute:'2-digit' })}</td>
